      ...
      <div className="w-full max-w-md animate-fadeIn">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 mb-4"
            ...
            <Wallet className="w-7 h-7" style={{ color: "var(--accent-ink)" }} />
          </div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: "var(--t1)" }}>
            {mode === "login" ? "Welcome back" : "Create your account"}
          </h1>
          <p className="mt-1 text-sm" style={{ color: "var(--t3)" }}>
            {mode === "login" ? "Sign in to manage your budget" : "Start tracking your finances today"}
          </p>
        </div>

        {/* Form card */}
        <div className="p-8" style={{ background: "var(--surface)", borderRadius: "var(--r-card)", boxShadow: "var(--shadow-card)", border: "1px solid var(--border)" }}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && (
              <div className="text-sm px-4 py-2.5" style={{ background: "var(--bad-tint)", color: "var(--bad)", borderRadius: "var(--r-field)" }}>{error}</div>
            )}
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                <div className="w-5 h-5 rounded-full border-2 animate-spin" style={{ borderColor: "var(--accent-ink)", borderTopColor: "transparent" }} />
              ) : (
            ...
            {mode === "login" ? (
              <>Don&apos;t have an account?{" "}
                <button onClick={() => { setMode("signup"); setError(""); }}
                  className="font-semibold hover:underline" style={{ color: "var(--accent-dim)" }}>Sign up</button>
              </>
            ) : (
              <>Already have an account?{" "}
                <button onClick={() => { setMode("login"); setError(""); }}
                  className="font-semibold hover:underline" style={{ color: "var(--accent-dim)" }}>Sign in</button>
              </>
            )}
          </div>
        </div>
        ...
        <p className="text-center text-xs mt-6" style={{ color: "var(--t3)" }}>
          ...
          <Link href="/terms" className="underline hover:opacity-70">Terms</Link> and{" "}
...
function FieldGroup({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="relative">
        <span className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "var(--t3)" }}>{icon}</span>
        {children}
      </div>
    </div>
  );
