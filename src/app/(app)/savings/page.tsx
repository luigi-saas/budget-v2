    ...
    <div className="p-5" style={{ background: "var(--surface)", borderRadius: "var(--r-card)", border: "1px solid var(--border)", boxShadow: "var(--shadow-card)", opacity: goal.active ? 1 : 0.6 }}>
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--t3)" }}>Source: {goal.source}</p>
        </div>
        <div className="flex items-center gap-1.5">
          {completed && <Check className="w-4 h-4" style={{ color: "var(--good)" }} />}
          <button onClick={onFund} className="p-1.5 transition hover:opacity-70"
            style={{ background: "var(--accent-tint)", borderRadius: "var(--r-field)" }}>
            <TrendingUp className="w-3.5 h-3.5" style={{ color: "var(--accent-dim)" }} />
          </button>
          <button onClick={onToggle} className="p-1.5 transition hover:opacity-70"
          ...
          <button onClick={onDelete} className="p-1.5 transition hover:opacity-70"
            style={{ background: "var(--bad-tint)", borderRadius: "var(--r-field)" }}>
      ...
      <div className="mb-3">
        <div className="flex justify-between text-xs mb-1.5">
          <span style={{ color: "var(--t2)" }}>{formatCurrency(goal.current, currency)} saved</span>
          <span style={{ color: "var(--t3)" }}>of {formatCurrency(goal.target, currency)}</span>
        </div>
        <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--surface-2)" }}>
          <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: completed ? "var(--good)" : "var(--accent)" }} />
        </div>
      </div>
      <p className="text-xs font-medium" style={{ color: completed ? "var(--good)" : "var(--t3)" }}>
        {completed ? "🎉 Goal reached!" : `${pct.toFixed(1)}% complete`}
      </p>
    </div>
  );
}
