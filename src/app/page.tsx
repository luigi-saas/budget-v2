...
import { Wallet, TrendingUp, PiggyBank, Shield, ArrowRight, BarChart3, Globe, Smartphone } from "lucide-react";

export default function LandingPage() {
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      {/* Nav */}
      <nav className="sticky top-0 z-50 backdrop-blur-md" style={{ background: "rgba(255,255,255,0.85)", borderBottom: "1px solid var(--border)" }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 h-16">
          ...
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm font-medium px-4 py-2 transition-all hover:opacity-70"
              style={{ color: "var(--t2)", borderRadius: "var(--r-field)" }}>
              Sign In
            </Link>
            <Link href="/login" className="text-sm font-semibold px-5 py-2 transition-all hover:opacity-90"
      ...
      <section id="features" className="py-24" style={{ background: "var(--surface-2)", borderTop: "1px solid var(--border)" }}>
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold" style={{ color: "var(--t1)" }}>Everything you need to budget</h2>
            <p className="mt-3 text-lg" style={{ color: "var(--t3)" }}>A complete toolkit for managing your personal finances</p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              { icon: Shield, title: "Private & Secure", desc: "Your data stays yours. Server-side auth with encrypted passwords.", tint: "var(--accent-tint)" },
            ].map(f => (
      ...
      <section className="py-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          <div className="p-12 sm:p-16" style={{ background: "var(--accent)", borderRadius: "calc(var(--r-card) + 4px)", boxShadow: "var(--shadow-btn)" }}>
            <h2 className="text-3xl sm:text-4xl font-bold" style={{ color: "var(--accent-ink)" }}>Ready to take control?</h2>
            <p className="mt-4 text-lg opacity-70" style={{ color: "var(--accent-ink)" }}>Join thousands who already track their finances with Flousy.</p>
      ...
      <footer className="py-10 px-6" style={{ borderTop: "1px solid var(--border)" }}>
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 text-sm" style={{ color: "var(--t3)" }}>
            ...
            <Link href="/help" className="hover:opacity-70 transition">Help</Link>
          </div>
          <p className="text-xs" style={{ color: "var(--t3)" }}>&copy; {new Date().getFullYear()} Flousy. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
